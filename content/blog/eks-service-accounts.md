...
on what AWS is actually doing, and what I believe they did wrong.
...
but I was yet to find a reference to OIDC endpoints and Kubernetes.  Googling K8s and OIDC just 
brings you a load of references to federated user authentication, but nothing on Service Accounts. 
Eventually I was able to stumble across a few PRs.  The first in 
...
##### The AWS SDK
The final tooling needed for this to work was to updte the SDKs.  Normally the SDKs look in your .aws
folder or in your AWS_* environment variables to authenticate against the AWS API. 
...
to IAM, with or without Kuberentes.  I won't spend a lot of time on this, but you need to [add the 
provider to  IAM](https://docs.aws.amazon.com/eks/latest/userguide/enable-iam-roles-for-service-accounts.html). 

##### Create an IAM Role
I am going to give an example of an IAM Role as it would look in Cloudformation (YAML FTW)
```yaml
  S3ReadBuckets:
    Type: 'AWS::IAM::Role'
    Properties:
...
```system:serviceaccount:my-namepace:my-serviceaccount``` can assume this role.

##### Create pod a pod with a role.
Now we will creat a pod with the Projected Service Account and let the AWS SDK know how to consume 
it.
...
had set an env variable already it would not be overwritten.
...
There is a clear seperation between authentication and authorization.

###### They Misrepresent Permissions
...
the role arn as an env variable on the pod.

##### Giving back to the community
It would have been nice to see higher engagement with the K8s community through code contribution.
It seeems in a rush to provide user features they shot for short term gains, without providing back
