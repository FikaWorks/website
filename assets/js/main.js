  ...
  if (filterBtn) {
    ...
    const filterToggle = function() {
      if (document.querySelector("ul.filter-list").classList.toggle("hidden")) {
        filterBtn.classList = "relative w-full cursor-default rounded-lg py-4 px-6 text-left border-2 focus:outline-none bg-white border-gray-800 text-black";
      } else {
        filterBtn.classList = "relative w-full cursor-default rounded-lg py-4 px-6 text-left border-2 focus:outline-none bg-black border-black text-white";
      }
    };
    ...
    items.forEach(li => {
      li.addEventListener("click", function () {
        ...
        if (clickedDataValue === "") {
          ...
          allFilterableItems.forEach(item => {
            const shouldShow = item.classList.contains("filtered-item-" + clickedDataValue);
        ...
        items.forEach(menu_item => {
          ...
          if (menu_item.dataset.value === clickedDataValue) {
            menu_item.querySelector("svg").innerHTML = '<g clip-path="url(#single_select_selected_svg__a)"><rect x="4" y="4" width="16" height="16" rx="8" fill="#000"></rect><rect x="4" y="4" width="16" height="16" rx="8" stroke="#fff" stroke-width="2"></rect></g><rect x="1" y="1" width="22" height="22" rx="11" stroke="#000" stroke-width="2"></rect><defs><clipPath id="single_select_selected_svg__a"><rect width="24" height="24" rx="12" fill="#fff"></rect></clipPath></defs>';
          } else {
            menu_item.querySelector("svg").innerHTML = '<rect x="1" y="1" width="22" height="22" rx="11" fill="#fff"></rect><rect x="1" y="1" width="22" height="22" rx="11" stroke="#BDBDBD" stroke-width="2"></rect>';
